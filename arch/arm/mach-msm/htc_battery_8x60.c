					...
					char *buf)
{
	int len = 0;
		...
		len += scnprintf(buf + len, PAGE_SIZE - len,
			"cc:%d\n", (htc_batt_info.rep.batt_current -	htc_batt_info.rep.batt_discharg_current)*1000);
...
/*        if(htc_batt_mhl_dongle)*/
/*                htc_battery_set_charging(POWER_SUPPLY_DISABLE_CHARGE);*/

/*        BATT_LOG("%s POWER_SUPPLY_DISABLE_CHARGE, %d", __func__, htc_batt_mhl_dongle);*/
...
/*        BATT_LOG("%s POWER_SUPPLY_ENABLE_INTERNAL %d", __func__, htc_batt_mhl_dongle);*/
