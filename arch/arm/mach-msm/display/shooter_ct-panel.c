/* linux/arch/arm/mach-msm/display/shooter_ct-panel.c
 *
...
/*
TODO:
1. move regulator initialization to shooter_ct_panel_init()
*/
static struct regulator *l12_3v;
static struct regulator *lvs1_1v8;

static void shooter_ct_panel_power(int onoff)
{
	static int init;
	int ret;
	int rc;
...
static unsigned char shooter_ct_shrink_pwm(int val)
{
...
struct mdp_reg shooter_ct_color_v11[] = {
	{0x93400, 0x0222, 0x0},
	{0x93404, 0xFFE4, 0x0},
	{0x93408, 0xFFFD, 0x0},
	{0x9340C, 0xFFF1, 0x0},
...
int shooter_ct_mdp_color_enhance(void)
{
	PR_DISP_INFO("%s\n", __func__);
	/*mdp4_dsi_color_enhancement(shooter_ct_color_v11, ARRAY_SIZE(shooter_ct_color_v11));*/

	return 0;
}

int shooter_ct_mdp_gamma(void)
{
	PR_DISP_INFO("%s\n", __func__);
...
#endif
	.mdp_color_enhance = shooter_ct_mdp_color_enhance,
	.mdp_gamma = shooter_ct_mdp_gamma,
};
...
int __init shooter_ct_init_panel(struct resource *res, size_t size)
{
	...
	mipi_novatek_panel_data.shrink_pwm = shooter_ct_shrink_pwm;
...
static void shooter_ct_3Dpanel_on(bool bLandscape)
{
	...
	rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(SHOOTER_CT_3DLCM_PD), &pwm_gpio_config);
	if (rc < 0)
		pr_err("%s pmic gpio config gpio %d failed\n", __func__, PM8058_GPIO_PM_TO_SYS(SHOOTER_CT_3DLCM_PD));

	rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(SHOOTER_CT_3DCLK), &clk_gpio_config_on);
	if (rc < 0)
		pr_err("%s pmic gpio config gpio %d failed\n", __func__, SHOOTER_CT_3DCLK);
	...
	if (bLandscape) {
		...
		gpio_set_value(SHOOTER_CT_CTL_3D_1, 1);
		gpio_set_value(SHOOTER_CT_CTL_3D_2, 1);
		gpio_set_value(SHOOTER_CT_CTL_3D_3, 1);
		gpio_set_value(SHOOTER_CT_CTL_3D_4, 0);
	} else {
		mdp4_dsi_color_enhancement(mdp_sharp_barrier_on, ARRAY_SIZE(mdp_sharp_barrier_on));
		gpio_set_value(SHOOTER_CT_CTL_3D_1, 1);
		gpio_set_value(SHOOTER_CT_CTL_3D_2, 1);
		gpio_set_value(SHOOTER_CT_CTL_3D_3, 0);
		gpio_set_value(SHOOTER_CT_CTL_3D_4, 1);
	}
}

static void shooter_ct_3Dpanel_off(void)
{
	int rc;
	pwm_gpio_config.output_value = 0;
	rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(SHOOTER_CT_3DLCM_PD), &pwm_gpio_config);
	if (rc < 0)
		pr_err("%s pmic gpio config gpio %d failed\n", __func__, SHOOTER_CT_3DLCM_PD);
	mdp4_dsi_color_enhancement(mdp_sharp_barrier_off, ARRAY_SIZE(mdp_sharp_barrier_off));
	pwm_disable(pwm_3d);

	rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(SHOOTER_CT_3DCLK), &clk_gpio_config_off);
	if (rc < 0)
		pr_err("%s pmic gpio config gpio %d failed\n", __func__, SHOOTER_CT_3DCLK);
	gpio_set_value(SHOOTER_CT_CTL_3D_1, 0);
	gpio_set_value(SHOOTER_CT_CTL_3D_2, 0);
	gpio_set_value(SHOOTER_CT_CTL_3D_3, 0);
	gpio_set_value(SHOOTER_CT_CTL_3D_4, 0);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	switch (val) {
	case BARRIER_OFF:
		shooter_ct_3Dpanel_off();
		break;
	case BARRIER_LANDSCAPE:
		shooter_ct_3Dpanel_on(true);
		break;
	case BARRIER_PORTRAIT:
		shooter_ct_3Dpanel_on(false);
		break;
	default:
		break;
	}
...
static int shooter_ct_3Dpanel_probe(struct platform_device *pdev)
{
	int err = 0;
	printk(KERN_INFO "%s\n", __func__);
...
static int shooter_ct_3Dpanel_remove(struct platform_device *pdev)
{
	printk(KERN_INFO "%s\n", __func__);
	if (pwm_3d) {
		pwm_free(pwm_3d);
...
struct platform_driver shooter_ct_3Dpanel_driver = {
	.probe	= shooter_ct_3Dpanel_probe,
	.remove	= shooter_ct_3Dpanel_remove,
	.driver	= {
		.name = "panel_3d",
	},
};

static int __init shooter_ct_3Dpanel_init(void)
{
	pr_info("%s(%d)\n", __func__, __LINE__);
	return platform_driver_register(&shooter_ct_3Dpanel_driver);
}

static void __exit shooter_ct_3Dpanel_exit(void)
{
	platform_driver_unregister(&shooter_ct_3Dpanel_driver);
}

module_init(shooter_ct_3Dpanel_init);
module_exit(shooter_ct_3Dpanel_exit);
